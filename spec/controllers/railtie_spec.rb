...
describe TestController do
  ...
  #  - http://stackoverflow.com/questions/5474303/auth-hmac-functional-testing-problem
  def with_hmac_signed_requests(access_key_id, secret, &block)
    ...
    @controller.instance_eval %Q(
      ...
      def process_with_new_base_test(request, response)
        ApiAuth.sign!(request, '#{access_key_id}', '#{secret}')
  ...
  context "Controller test"  do
    ...
    it "should permit a request with properly signed headers" do
      ...
      # process(@request.params[:action], @request.path).code.should == "200"
      with_hmac_signed_requests('1044', API_KEY_STORE["1044"]) do
        get :index
